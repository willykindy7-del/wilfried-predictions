...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    body { background: #060D1A; font-family: 'Rajdhani', 'Tajawal', sans-serif; color: #F0F4FF; overflow-x: hidden; }
    ...
    .shimmer-gold {
      background: linear-gradient(90deg, #7a5c00 0%, #c9a000 15%, #FFD700 30%, #FFF5A0 45%, #FFD700 60%, #c9a000 75%, #7a5c00 100%);
...
const QUOTES = [
  { text: "Le risque vient de ne pas savoir ce que vous faites.", author: "Warren Buffett" },
  { text: "Ce qui compte c'est combien vous gagnez quand vous avez raison.", author: "George Soros" },
  { text: "L'argent se fait en attendant, pas en tradant.", author: "Jesse Livermore" },
  { text: "La discipline est la frontiÃ¨re entre les gagnants et les perdants.", author: "Peter Lynch" },
  { text: "Analyse. DÃ©cide. Gagne.", author: "WILFRIEDâ„¢" },
  { text: "N'investissez jamais dans une entreprise que vous ne pouvez pas comprendre.", author: "Warren Buffett" },
];
  ...
  return (
    <div style={{ textAlign: "center", padding: "30px 16px 14px", position: "relative" }}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.85 }}
        ...
        PrÃ©dictions â€¢ Intelligence Artificielle
      </motion.div>
...
const ACInput = ({ label, icon, value, onChange, suggestions, onSelect, loading, placeholder }) => {
  ...
  useEffect(() => { setOpen(suggestions.length > 0 && value.length >= 2); }, [suggestions, value]);
  useEffect(() => {
...
const FormSelector = ({ label, form, onChange }) => {
  const opts = ["V", "N", "D"];
  const colors = { V: "#00C853", N: "#FFA000", D: "#FF1744" };
  return (
    <div>
...
const ConfidenceMeter = ({ value }) => {
  const R = 50; const circ = 2 * Math.PI * R; const offset = circ - (value / 100) * circ;
  const color = value >= 90 ? "#00C853" : value >= 75 ? "#FFA000" : "#FF1744";
  return (
    <div style={{ position: "relative", width: 130, height: 130, margin: "0 auto" }}>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.9, type: "spring", stiffness: 220 }}
        ...
        <div style={{ fontSize: "0.58rem", color: "rgba(240,244,255,0.5)", letterSpacing: "0.12em", fontFamily: "Rajdhani" }}>PRÃ‰CISION</div>
      </motion.div>
    </div>
  );
};
...
const BottomNav = ({ active, onChange }) => {
  const tabs = [{ id: "home", icon: "ðŸ ", label: "Accueil" }, { id: "history", icon: "ðŸ“‹", label: "Historique" }, { id: "programme", icon: "ðŸ“…", label: "Programme" }];
  return (
...
const HistoryItem = ({ item, onUpdate }) => {
  ...
  const sl = item.status === "won" ? "âœ… GagnÃ©" : item.status === "lost" ? "âŒ Perdu" : "â³ En attente";
  return (
    <motion.div initial={{ opacity: 0, x: -18 }} animate={{ opacity: 1, x: 0 }} className="glass-card" style={{ padding: 14, marginBottom: 10, borderColor: `${sc}35` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
        <div>
          ...
          <div style={{ fontSize: "0.75rem", color: "rgba(240,244,255,0.4)", fontFamily: "Rajdhani" }}>{item.competition} Â· {new Date(item.date).toLocaleDateString("fr-FR")}</div>
        </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        {item.status === "pending" && (
          <div style={{ display: "flex", gap: 6 }}>
            {[{ s: "won", l: "âœ… GagnÃ©", c: "#00C853" }, { s: "lost", l: "âŒ Perdu", c: "#FF1744" }].map(({ s, l, c }) => (
    ...
    <div className="gold-card" style={{ padding: 16, margin: "14px 0", borderLeft: "3px solid #FFD700" }}>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={idx} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.45 }}>
          ...
          <div style={{ fontSize: "0.78rem", color: "#1E90FF", fontFamily: "Rajdhani", fontWeight: 700 }}>â€” {QUOTES[idx].author}</div>
        </motion.div>
      </AnimatePresence>
    </div>
  );
...
export default function App() {
  ...
  const [homeStats, setHomeStats] = useState({ scored: "", conceded: "" });
  const [awayStats, setAwayStats] = useState({ scored: "", conceded: "" });
  ...
  const handleAC = (type, val) => {
    if (type === "home") setHomeTeam(val);
    else if (type === "away") setAwayTeam(val);
    else setCompetition(val);
    if (timers.current[type]) clearTimeout(timers.current[type]);
    if (val.length < 2) { setSugg((s) => ({ ...s, [type]: [] })); return; }
    timers.current[type] = setTimeout(async () => {
      setAcLoad((l) => ({ ...l, [type]: true }));
      try {
        const sys = "RÃ©ponds UNIQUEMENT avec un tableau JSON valide, sans markdown ni backticks.";
        const prompt = type === "comp"
          ? `Liste 6 compÃ©titions de football professionnelles contenant "${val}". JSON: [{"name":"...","detail":"ConfÃ©dÃ©ration"}]`
          : `Liste 6 clubs de football ou sÃ©lections contenant "${val}". JSON: [{"name":"...","flag":"ðŸ³ï¸","detail":"Pays/Ligue"}]`;
  ...
  const generate = async () => {
    if (!homeTeam || !awayTeam) { setError("Veuillez saisir les deux Ã©quipes."); return; }
    setLoading(true); setError(""); setPred(null);
    const formStr = (f) => f.map((x) => x || "?").join("-");
    try {
      const sys = `Tu es WILFRIEDâ„¢, analyste football IA d'Ã©lite ultra-prÃ©cis. Tu rÃ©ponds UNIQUEMENT en JSON valide sans markdown ni backticks. Tes prÃ©dictions sont dÃ©taillÃ©es, confiantes et structurÃ©es. Tu rÃ©ponds en franÃ§ais.`;
      const prompt = `Analyse ce match :
ðŸ  Domicile: ${homeTeam} | âœˆï¸ ExtÃ©rieur: ${awayTeam}
ðŸ† CompÃ©tition: ${competition || "Non prÃ©cisÃ©e"}
ðŸ“Š Cotes 1XBET: 1=${odds.home || "N/A"} X=${odds.draw || "N/A
